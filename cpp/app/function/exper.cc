...
public:
    ...
    ExperExr(std::string test_data_name,int query_index){
        ...
        this->base_qfile = "../data/"+test_data_name+"/query4/q";
...
public:
  ...
  void generate_query_base_dgraph(int generate_query_nodes, int max_calculate_center_nodes,int generate_query_nums){
      ...
      //std::fstream tmp_outfile("../data/yago/query5_/query_info.txt",std::ios::out);
      //std::fstream tmp_outfile("../data/pokec/query4/query_info.txt",std::ios::out);
      std::fstream tmp_outfile("../data/liveJournal/query4/query_info.txt",std::ios::out);
      tmp_outfile.close();
      while(i<=generate_query_nums){
          ...
          if(max_dual_set.size()<=max_calculate_center_nodes){
              ...
              //std::fstream tmp_outfile("../data/yago/query5_/query_info.txt",std::ios::app);
              //std::fstream tmp_outfile("../data/pokec/query4/query_info.txt",std::ios::app);
              std::fstream tmp_outfile("../data/liveJournal/query4/query_info.txt",std::ios::app);
              tmp_outfile<<i<<' '<<"calculate dual time="<<(float)(e0-s0)/CLOCKS_PER_SEC<<"s"<<"  ball num="<<max_dual_set.size()<<"  dQ="<<d_Q<<std::endl;
              tmp_outfile.close();
              i++;
          }
      }
...
void first_strong_strongInc(int circle_num, int flag){
	...
	std::vector<StrongR> strongsimr = strongsim.strong_simulation_sim_only_add(dgraph, qgraph, 1, whole_dist);
    clock_t e0 = clock();
   ...
   while (j<=circle_num){
	    clock_t tmps1 = clock();
		std::unordered_map<VertexID,std::unordered_map<VertexID,int>> tmp_whole_dist;
          for(auto it:whole_dist){
              tmp_whole_dist[it.first] = it.second;
          }

		clock_t tmpe1 = clock();
		std::cout<<"whole_dist.size="<<whole_dist.size()<<", tmp_whole_dist.size="<<tmp_whole_dist.size()<<std::endl;
		std::cout<<"copy tmp_whole_dist : "<<(float)(tmpe1-tmps1)/CLOCKS_PER_SEC<<"s"<<std::endl;
		...
		if(flag == 1){
			stronginc.strong_simulation_inc_only_add(dgraphinc,qgraph,tmp_sim,tmp_r,add_edges,1,tmp_whole_dist);
        }
		clock_t end2 = clock();
...
//  ExperExr experExr(base_name,8);
  ExperExr experExr(base_name,19);

//  experExr.generate_query_base_dgraph(4,7000,500);  // 生成查询图Q。   (点数，生成最大ball数，生成Q的数量)
  experExr.generate_all_random_edges(3666148,18,50,20); // 完全随机生成增减边。（每次增加或者减少的量，增加和减少的次数，新增的点占delta(G)的比例，增删边的重叠比例）
